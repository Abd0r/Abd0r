# GitHub Sponsors profile copy (README-matched)

Paste the **Full description** block into:
https://github.com/sponsors/Abd0r/dashboard  
→ Profile → Introduction / full description

Paste the **Short description** into the short blurb field (replaces the default  
`Support ra2157218-boop's open source work`).

Assets live in this repo (`Abd0r/Abd0r`) so paths match the profile README.
...
## Suggested tiers (create in Sponsors dashboard → Tiers)

| Amount | Name | Description |
|---|---|---|
| **$1 / mo** | `pixel spark` | Thanks + name on the sponsor wall. Funds a coffee-or-two of cloud credits. |
| **$5 / mo** | `KV cache` | Helps cover dataset storage + small training runs. |
| **$15 / mo** | `SAVO cell` | Meaningful compute share toward NanoG1 / Quatrix ablations. |
| **$50 / mo** | `research cell` | Serious monthly compute. Optional: early notes / shout-out on X when you want one. |
| **$5 one-time** | `one-shot spark` | One-time thank-you. Same spirit, no subscription. |
| **$25 one-time** | `one-shot run` | Covers a focused experiment or paper revision sprint. |

Tier body copy can reuse the short lines above; keep perks honest (no private product promises).

---

## API note

Updating `fullDescription` / creating tiers via GraphQL needs a `user` (or `admin:org`) scope. Current `gh` token only has `gist read:org repo workflow`. Dashboard paste works without that.
